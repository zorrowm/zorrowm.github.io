...
class CollisionsDemo {
  public static CreateScene(engine: BABYLON.Engine, canvas: HTMLCanvasElement): BABYLON.Scene {
    ...
    new Collisions(engine, scene, ground);
    //return scene;
    return undefined;
  }
}
...
class Collisions {
  ...
  private _intersectsMeshByG(mesh: Mesh, mesh2: Mesh): boolean {
    return mesh.content.position.y - mesh.size.height / 2 <
      mesh2.content.position.y + mesh2.size.height / 2
      ? true
      : false;
  }
  ...
  protected _init(): void {
    ...
    this.engine.runRenderLoop(() => {
      ...
      for (let i = 0; i < this.meshes.length; i++) {
        ...
        const material: BABYLON.StandardMaterial = mesh.content
